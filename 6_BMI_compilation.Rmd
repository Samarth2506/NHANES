...
# subject scores + abs(PC scores)
low_BMI_thre = quantile(analyticData$BMI,probs = c(0.2,0.8))[1]
high_BMI_thre = quantile(analyticData$BMI,probs = c(0.2,0.8))[2]

BMI_id_low = analyticData$SEQN[which(analyticData$BMI < low_BMI_thre)]
BMI_id_high = analyticData$SEQN[which(analyticData$BMI > high_BMI_thre)]

BMI_low = MINdata %>% filter(SEQN %in% BMI_id_low) %>% select(-SEQN)
BMI_high = MINdata %>% filter(SEQN %in% BMI_id_high) %>% select(-SEQN)

pcscores_lowbmi = scale(BMI_low, pca_model$center, pca_model$scale) %*% pca_model$rotation
pcscores_lowbmi = cbind('SEQN' = BMI_id_low,pcscores_lowbmi) %>% as.data.frame()

pcscores_highbmi = scale(BMI_high, pca_model$center, pca_model$scale) %*% pca_model$rotation
pcscores_highbmi = cbind('SEQN' = BMI_id_high,pcscores_highbmi) %>% as.data.frame()

# box plot

pcscores = rbind(cbind(pcscores_highbmi, BMI = 'high_BMI'), 
                 cbind(pcscores_lowbmi, BMI = 'low_BMI'))
gs = lapply(1:3, function(i) {
  ggplot(pcscores) +
    geom_boxplot(aes(x = BMI, y = pcscores[,paste0('PC',i)] %>% unlist %>% abs)) +
    labs(y = paste0('Absolute PC',i,' Scores'))
})

do.call(grid.arrange,c(gs,nrow = 1))



# subject score + abs(pc scores)

gs = lapply(1:3, function(i) {
  temp_high = MINdata %>% filter(SEQN %in% BMI_id_high) %>% select(-SEQN)  + abs(pcscores_highbmi[,paste0('PC',i)])
  temp_high = temp_high  %>% colMeans() %>% as.data.frame()
  
  temp_low = MINdata %>% filter(SEQN %in% BMI_id_low) %>% select(-SEQN) + abs(pcscores_lowbmi[,paste0('PC',i)])
  temp_low = temp_low  %>% colMeans() %>% as.data.frame()
return(
  ggplot() +
    geom_line(data = temp_high, aes(x = 1:nrow(temp_high)/60, y = temp_high[1] %>% unlist(), color = 'Scores of Subjects with High BMI(>30)')) + 
    geom_line(data = temp_low, aes(x = 1:nrow(temp_low)/60, y = temp_low[1] %>% unlist(), color = 'Scores of Subjects with Low BMI(<20)')) + 
    labs(x = "Time of Day", y = paste0("Subject Scores + PC",i," Scores")) +
