...
plot(( 1 : nrow(temp)) / 60, temp[,9],type = 'l',
     xlab = "Time of Day",ylab = "Variable Loadings on PC5")
plot(( 1 : nrow(temp)) / 60, temp[,10],type = 'l',
     xlab = "Time of Day",ylab = "Variable Loadings on PC5")
```
...
for(i in 1:30){
  ...
  dat$BMI = log(dat$BMI + 1) # here we don't transform back BMI when calculating error
  ...
  save(res_RMSE,res_Rsquared, file = 'CV_NumberofPCs.rda')
}
load(file = 'CV_NumberofPCs.rda')
...
fit = glm(BMI ~ .,data = y, subset = trainidx)
