...
ggplot(data.frame(temp)) + 
  aes(x = 1:nrow(temp)/60,y = temp[,1]) + 
  geom_line() +
  labs(x = "Time of Day", y = "Variable Loadings on PC1")

ggplot(data.frame(temp)) + 
  aes(x = 1:nrow(temp)/60,y = temp[,2]) + 
  geom_line() +
  labs(x = "Time of Day", y = "Variable Loadings on PC2")

ggplot(data.frame(temp)) + 
  aes(x = 1:nrow(temp)/60,y = temp[,3]) + 
  geom_line() +
  labs(x = "Time of Day", y = "Variable Loadings on PC3")

ggplot(data.frame(temp)) + 
  aes(x = 1:nrow(temp)/60,y = temp[,4]) + 
  geom_line() +
  labs(x = "Time of Day", y = "Variable Loadings on PC4")

ggplot(data.frame(temp)) + 
  aes(x = 1:nrow(temp)/60,y = temp[,5]) + 
  geom_line() +
  labs(x = "Time of Day", y = "Variable Loadings on PC5")
```
...
ggplot(data.frame(res_RMSE)) + 
  ...
  labs(x = 'Number of PCs in Cross-Validation',y = 'RMSE')
...
ggplot(data.frame(res_Rsquared)) + 
  ...
  # geom_smooth(se = TRUE, method = 'lm')+
  geom_smooth(se = TRUE, method =  'auto',color = 'blue') + 
  labs(x = 'Number of PCs in Cross-Validation',y = 'R-squared')
