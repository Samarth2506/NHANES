...
# which(score$score_sum == max(score$score_sum))
# subject 1931 has the maximum of sum of scores


g = ggplot(gather(score[nrow(score),])) + 
  aes(x = 1:1440,y = value, color = "Average of Principal Component Scores") + 
  geom_line() + 
...
save(MINdata,MINdata_10080,file='MINdata.rda')
```
...
MINdata = as.data.frame(lapply(MINdata, function (x) if (is.factor(x)) unclass(x) %>% as.numeric else x))
